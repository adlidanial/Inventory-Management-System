...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * FROM asset WHERE asset_barcode = ?";
    $result = mysqli_prepare($connect, $sql);
    mysqli_stmt_bind_param($result, 's', $assetbarcode);
    ...
    if (!$asset) // if asset does not exists
    {
        ...
        $result = mysqli_prepare($connect, $sql);
        mysqli_stmt_bind_param($result, 's', $_SESSION['username']);
        mysqli_stmt_execute($result);
        $result = mysqli_stmt_get_result($result);
        $user = mysqli_fetch_assoc($result);
        $query = "INSERT INTO asset (fk_user_id, asset_barcode, name, price, date_created, date_modify) 
          VALUES(?, ?, ?, ?, now(), now())";
    
        $result = mysqli_prepare($connect, $query);
        mysqli_stmt_bind_param($result, 'issd', $user['user_id'], $assetbarcode, $assetname, $price);
        ...
        if($result)
        {
            ...
            $query = "INSERT INTO list_record (fk_asset_id, fk_status_id, description, date_created, date_modify) 
            VALUES(?, ?, ?, now(), now())";
            $text = "";
            $num = 1;
            $result = mysqli_prepare($connect, $query);
            mysqli_stmt_bind_param($result, 'iis', $asset['asset_id'], $num, $text);
            $result = mysqli_stmt_execute($result);

            if($result)
            {
                echo ("<script>
                window.alert('Succesfully insert new asset.');
                window.location.href='asset.php';
                </script>");
            }
            else
            {
                var_dump($asset['asset_id']);
                exit();
            }
    ...
    <main class="page lanidng-page">
        <section class="portfolio-block block-intro">
            <div class="container">
                ...
                <hr>
                <div>
                    <form style="background-color: #ffffff;" method="post" action="add_asset.php">
                        <div class="form-group">
                            <p style="text-align:center"><video id="video" width="400" height="200" autoplay></video></p>
                            <button class="btn btn-secondary border rounded" type="button">Snap Barcode</button></div>
                        <div class="form-group text-left">
                            <label>Barcode ID</label>
                            <input class="form-control" type="text" name="assetbarcode" required>
                        </div>
                        <div class="form-group text-left">
                            <label>Name</label>
                            <input class="form-control" type="text" name="assetname" required>
                        </div>
                        <div class="form-group text-left">
                            <label>Price</label>
